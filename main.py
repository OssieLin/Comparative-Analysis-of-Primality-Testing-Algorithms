...
list_of_spp = [[log_of_number_of_lucas_type_pseudoprime_up_to(c0, c1, 3, 1000) for c0 in range(c0_min, c0_max)] for c1 in range(c1_min, c1_max)]
plt.imshow(list_of_spp, extent=(c0_min, c0_max, c1_min, c1_max))
colorbar = plt.colorbar()

plt.title('Logs of Number of Lucas-Type Pseudoprimes up to 1000 with 3-rough')
plt.xlabel('Parameter c0')
plt.ylabel('Parameter c1')
plt.xticks(range(c0_min, c0_max+1 ))
plt.yticks(range(c1_min, c1_max+1 ))
plt.show()
